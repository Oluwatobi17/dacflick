...
from index.models import Event, Todotask, Contact, Wall, Likepage, User, Following, Notification, View, Favour
from .serializers import EventSerializer, TodotaskSerializer, ContactMsgSerializer, PostCommentSerializer, AddPostSerializer, PostSerializer, WallSerializer, WallReplySerializer, FollowingSerializer, UserSerializer
...
class Commentpost(APIView):
	...
	def post(self, request):
		...
		if serializer.is_valid():
			...
			newData['comDate'] = datetime.datetime.now()
...
class CreateEvent(APIView):
	...
	def post(self, request):
		...
		if newevent.is_valid():
			...
			return Response(newevent.data['id'])
		else:
			return Response(False)
...
class AllEvent(APIView):
	...
	def get(self, request):
		user = User.objects.get(username=request.user.username)
		data = Event.objects.filter(user=user).order_by('-date')
		serializer = EventSerializer(data, many=True)

		return Response(serializer.data)

		
class AddTodoTask(APIView):
	permission_classes = (IsAuthenticated, )
	def post(self, request):
...
class DeleteTodoTask(APIView):
	permission_classes = (IsAuthenticated,)
	def get(self, request, id):
		Todotask.objects.filter(id=id).delete()

		return Response(id)


class CheckTodoTask(APIView):
	permission_classes = (IsAuthenticated,)
	def get(self, request, id):
		if Todotask.objects.filter(id=id):
...
class GetTasks(APIView):
	...
	def get(self, request, id):
		if Event.objects.filter(id=id):
			...
			serializer = TodotaskSerializer(event.todotask_set.all(), many=True)
...
class DeletePost(APIView):
	...
	def get(self, request, id):
		if Post.objects.filter(pk=id):
			if Post.objects.get(pk=id).user.username == request.user.username:
