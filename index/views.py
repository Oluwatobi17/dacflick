...
from .models import Event,Following, Wall, WallReply, Quoteoftheday, Jokeoftheday, Motivationoftheday
...
def myprofile(request):
	if not request.user.is_authenticated:
		...
		if Event.objects.filter(user=user):
			events = Event.objects.filter(user=user).order_by('-date')[:4]
	...
	if request.method=='POST':
		...
		if username:
			if User.objects.filter(username=username):
				...
				message = "Hello, {}. Your new password is {}. \n Please ignore this message if you don't have a Flick account".format(user.first_name, newpass)
				
				res = send_mail('Password reset', message, "support@flick.com",[user.email], fail_silently=True)
				return redirect(login)
			else:
...
def changepassword(request):
	...
	if user.check_password(currentPassword):
		...
		messages.error(request, 'Current password given does not match, try again')
		return redirect(myprofile)
